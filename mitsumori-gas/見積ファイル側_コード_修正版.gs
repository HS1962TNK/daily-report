...
/**
 * 表紙の「見積NO」「見積作成日」のセルを見分ける（母艦の coverCells_ と同じ判定）。
 * 2026年7月より前のレイアウトは右上ブロックが1列左にずれていて、見積NOがK2・作成日がK3にある。
 * 新しいレイアウトはL2・L3（K2は見出し「見積NO.」）。
 * 古い見積を複製したファイルにもこのスクリプトがそのままコピーされるので、
 * L2固定で読むと空になり「見積一覧に見つかりません」になっていた（2026-09-26）。
 */
function quoteCells_(cover) {
  const NEW = { no: 'L2', date: 'L3' };
  const OLD = { no: 'K2', date: 'K3' };
  const norm = function (a1) { return String(cover.getRange(a1).getValue() || '').replace(/[\s　]/g, ''); };
  const looksLikeNo = function (a1) { const s = norm(a1); return /^\d{6}-\d{3}$/.test(s) || /^\d{8}$/.test(s); };
  if (norm('K2').indexOf('見積NO') === 0) return NEW;
  if (norm('J2').indexOf('見積NO') === 0) return OLD;
  if (looksLikeNo('L2')) return NEW;
  if (looksLikeNo('K2')) return OLD;
  return NEW;
}
...
function updateQuoteIndex() {
  ...
  try {
    ...
    const cells = quoteCells_(cover);
    const quoteNo = String(cover.getRange(cells.no).getValue() || '').trim();
    if (!quoteNo) { ui.alert('表紙に見積NOが入っていません。'); return; }
    const quoteDate = cover.getRange(cells.date).getValue();
    const client = cover.getRange('B5').getValue();
    const jobName = cover.getRange('C9').getValue();
    const department = cover.getRange('J10').getValue();
    const person = cover.getRange('L10').getValue();
    const total = cover.getRange('C7').getValue();
    const url = thisSs.getUrl();

    const data = indexSheet.getDataRange().getValues();
    let row = 0;
    for (let i = 1; i < data.length; i++) {
      if (String(data[i][0]).trim() === quoteNo) { row = i + 1; break; }
    }
    // 一覧に無ければ行を足す（母艦の自動同期と同じ動き）
    const isNew = !row;
    if (isNew) {
      row = indexSheet.getLastRow() + 1;
      indexSheet.getRange(row, 1).setValue(quoteNo);
    }
    indexSheet.getRange(row, 2).setValue(quoteDate);
    indexSheet.getRange(row, 3).setValue(client);
    indexSheet.getRange(row, 4).setValue(jobName);
    indexSheet.getRange(row, 5).setValue(department);
    indexSheet.getRange(row, 6).setValue(person);
    indexSheet.getRange(row, 7).setFormula('=HYPERLINK("' + url + '","開く")');
    indexSheet.getRange(row, 8).setValue(total);
    ui.alert(isNew ? '見積NO「' + quoteNo + '」を見積一覧に登録しました。' : '見積一覧を更新しました。');
  } catch (e) {
    ui.alert('エラーが発生しました：' + e.message);
  }
}

// 複製は母艦・ポータル側で行う（見積NOの自動採番・年フォルダ保存・一覧登録をするため）。
function duplicateThisQuote() {
  SpreadsheetApp.getUi().alert(
    '見積の複製は「見積・請求ポータル」から行ってください。\n\n' +
    'ポータルの見積一覧で、この見積の行の「複製」ボタンを押すと、\n' +
    '新しい見積NOが自動で振られて見積一覧にも登録されます。');
}
